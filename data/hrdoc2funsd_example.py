...
def convert_hrdoc_to_funsd(hrdoc_dir, output_dir, split_name="train"):
    ...
    Args:
        hrdoc_dir: HRDoc 数据根目录（包含 train/test 子目录和 images 目录）
        output_dir: 输出目录
        ...
        for page_id in sorted(pages.keys()):
            ...
            if not img_found:
                ...
                if common_img_dir.exists():
                    for paper_img_dir in common_img_dir.iterdir():
                        if not paper_img_dir.is_dir():
                            continue
                        for ext in ['.jpg', '.png', '.jpeg']:
                            src_img = paper_img_dir / f"{page_name}{ext}"
                            if src_img.exists():
                                dst_img = img_out / f"{page_name}{ext}"
                                shutil.copy(src_img, dst_img)
                                img_found = True
...
def collect_labels(hrdoc_dir, split_name="train"):
    """
    收集所有标签，生成 labels.txt

    Args:
        hrdoc_dir: HRDoc 数据根目录
    ...
    if json_dir.exists():
        ...
        json_files = hrdoc_path.glob("*.json")
