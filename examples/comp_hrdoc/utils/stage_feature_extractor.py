...
def get_stage_feature_extractor(
    ...
    """
    工厂函数：获取 StageFeatureExtractor 实例

    Args:
        checkpoint_path: 显式指定的 checkpoint 路径
        env: 环境 (dev/test)，用于自动查找 checkpoint
        exp: 实验 ID，用于自动查找 checkpoint
        device: 计算设备

    Returns:
        StageFeatureExtractor 实例
    """
    if checkpoint_path is None:
        # 自动查找最新的 joint checkpoint
        from models.build import get_latest_joint_checkpoint

        # 加载配置
        sys.path.insert(0, os.path.join(_STAGE_ROOT, "util"))
        from config_setup import load_config_safe

        config = load_config_safe(env)
        checkpoint_path = get_latest_joint_checkpoint(config, exp=exp)

        if checkpoint_path is None:
            raise ValueError(
                f"No joint checkpoint found for env={env}, exp={exp}. "
                "Please specify checkpoint_path explicitly."
            )

    return StageFeatureExtractor(
