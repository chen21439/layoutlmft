...
Usage:
    ...
    # Stage1 + Construct 联合训练（端到端）
    python examples/comp_hrdoc/scripts/train_doc.py --env test --train-stage1 --layoutxlm-path /path/to/joint/checkpoint
    python examples/comp_hrdoc/scripts/train_doc.py --env test --train-stage1 --dataset hrds --quick
"""
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="hrds", choices=["hrds", "hrdh", "tender"],
                        help="Dataset to use when --train-stage1 is enabled")
...
def convert_stage_labels_to_construct(
    ...
    Returns:
        ...
        sibling_labels: [num_docs, max_lines] 左兄弟索引（自指向表示无左兄弟）
    ...
    # sibling_labels: 索引形式 [B, N]，存储每个节点的左兄弟索引
    # 论文自指向方案：无左兄弟时指向自己（sibling_labels[i] = i）
    sibling_labels = torch.arange(max_lines, device=device).unsqueeze(0).expand(num_docs, -1).clone()
...
def main():
    ...
    # 使用 StageFeatureExtractor 进行端到端训练，backbone 参与梯度更新
    stage_feature_extractor = None  # Used in train_stage1 mode
    if args.train_stage1:
        ...
        def save_stage1_model(construct_model, save_path, feature_extractor=stage_feature_extractor, tok=tokenizer):
            ...
            # 保存 line_enhancer 权重（行间特征增强 Transformer）
    ...
    # ==================== Standard Mode ====================
        ...
        tokenizer = LayoutXLMTokenizerFast.from_pretrained(model_name)
    ...
    # train_stage1 使用 ConstructFromFeatures，从 model_name_or_path 加载
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        # Train
        if args.train_stage1:
            # train_stage1 使用 train_epoch_with_stage_features
