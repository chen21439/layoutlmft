...
# Stage 1
python examples/stage/scripts/train_stage1.py --env test --quick --dataset hrds  --new_exp --restart
python examples/stage/scripts/train_stage1.py --env test  --dataset hrds --new_exp --restart
...
--use_cache true
--covmatch
nohup python examples/stage/scripts/train_stage1.py --env test --dataset hrds --init_from hrdh > logs/train_stage1_$(date +%Y%m%d_%H%M%S).log 2>&1 &
...
# 训练
  ...
  python examples/stage/train_joint.py \
      ...
      --model_path /data/LLM_group/layoutlmft/artifact/exp_20251225_180428/joint_hrds/checkpoint-2625 \
      --use_soft_mask false \
      --freeze_visual \
      > logs/train_joint_$(date +%Y%m%d_%H%M%S).log 2>&1 &
        ...
        --model_path /data/LLM_group/layoutlmft/artifact/exp_20251224_155832/joint_hrds/checkpoint-200 \
联合训练的模型格式
...
# 推理
  python examples/infer/run_joint_infer.py \
      --env test \
      --dataset tender \
      --model_path /data/LLM_group/layoutlmft/artifact/exp_20251216_231629/joint_tender/checkpoint-1050 \
      --skip_eval


```
  python examples/stage/scripts/create_dev_covmatch.py --env test --dataset tender --dev_ratio 0.125 --seed 42
