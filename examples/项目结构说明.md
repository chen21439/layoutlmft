...
训练/推理/评估 都需要考虑能不能复用
stage1单独训练/联合训练 也需要考虑能不能复用

validation(训练时)/test(训练好以后) eval
  ...
  │  训练 (train_joint.py)                                                  │
  │    └── JointModel.forward()                                            │
  │          ├── self.line_pooling(hidden_states) ──────┐                  │
  │          ├── self.cls_head(line_features) ──────────┼── 共享模块        │   │                  │
  │                                                     │                  │
  │  推理/评估 (predictor.py → evaluator.py)            │                  │
  │    └── Predictor.predict()                          │                  │
