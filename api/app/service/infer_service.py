...
class InferenceService:
    ...
    def predict_with_construct(
        ...
        if loader.has_construct_model:
            ...
            if format_b and format_b.get("pred_parents"):
                ...
                # [验证] 层级信息 Probing 分析
                # ============================================================
                section_indices = [features["line_ids"].index(p["line_id"]) for p in section_preds]
                self._analyze_hierarchy_probing(
                    section_preds=section_preds,
    ...
    def _analyze_hierarchy_probing(
        self,
        section_preds: List[Dict],
        line_features: torch.Tensor,
        section_indices: List[int],
        format_b: Dict,
        task_dir: str,
    ):
        """分析层级信息是否被 embedding 编码

        验证目标：
        1. 解析每个 section 的层级（册/章/节）
        2. 计算特征相似度，比较同层级 vs 跨层级
        3. 保存 probing 数据用于离线训练探针
        """
        import re
        import numpy as np
        import torch.nn.functional as F

        def parse_hierarchy_level(text: str) -> Tuple[str, int]:
            """解析文本的层级单位和级别

            Returns:
                (unit, level): 如 ('册', 1), ('章', 2), ('节', 3), ('unknown', 0)
            """
            text = text.strip()
            patterns = [
                (r'^第[一二三四五六七八九十百千\d]+册', '册', 1),
                (r'^第[一二三四五六七八九十百千\d]+编', '编', 1),
                (r'^第[一二三四五六七八九十百千\d]+部分', '部分', 1),
                (r'^第[一二三四五六七八九十百千\d]+章', '章', 2),
                (r'^第[一二三四五六七八九十百千\d]+节', '节', 3),
                (r'^第[一二三四五六七八九十百千\d]+条', '条', 4),
                (r'^[一二三四五六七八九十]+[、.]', '一级序号', 3),
                (r'^\([一二三四五六七八九十\d]+\)', '括号序号', 4),
                (r'^\d+\.\d+\.\d+', '三级编号', 4),
                (r'^\d+\.\d+', '二级编号', 3),
                (r'^\d+[、.]', '一级编号', 3),
            ]
            for pattern, unit, level in patterns:
        ...
        units = []
        texts = []
        for pred in section_preds:
            text = pred.get("text", "")
            unit, level = parse_hierarchy_level(text)
            levels.append(level)
            units.append(unit)
            texts.append(text[:30])  # 截断用于显示
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("[Probing 验证] 层级信息分析")
        logger.info("=" * 60)
        logger.info(f"层级分布: {dict(level_counter)}")
        logger.info(f"单位分布: {dict(unit_counter)}")

        # 2. 打印每个 section 的层级解析结果
        logger.info("")
        logger.info("--- 层级解析详情 ---")
        for i, pred in enumerate(section_preds):
            ...
            level = levels[i]
            # 预测的父节点
            pred_parent = format_b["pred_parents"][i] if i < len(format_b["pred_parents"]) else -1
            parent_line_id = format_b["section_line_ids"][pred_parent] if pred_parent != i and pred_parent >= 0 else -1
            is_root = pred_parent == i
            logger.info(
                f"  [{i:2d}] line_id={line_id:4d} | level={level} unit={unit:6s} | "
                f"parent={'ROOT' if is_root else parent_line_id:>4} | {text}"
            )

        # 3. 计算特征相似度分析
        if len(section_indices) > 1:
            # 提取 section 的特征向量
            section_feats = line_features[section_indices]  # [S, H]
            # L2 归一化
            section_feats_norm = F.normalize(section_feats, p=2, dim=-1)
            # 计算余弦相似度矩阵
            sim_matrix = torch.mm(section_feats_norm, section_feats_norm.t())  # [S, S]
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    if levels[i] == levels[j] and levels[i] != 0:
                        same_level_sims.append(sim_matrix[i, j])
                    elif levels[i] != 0 and levels[j] != 0:
                        diff_level_sims.append(sim_matrix[i, j])

            logger.info("")
            logger.info("--- 特征相似度分析 ---")
            if same_level_sims:
                logger.info(f"同层级相似度: mean={np.mean(same_level_sims):.4f}, std={np.std(same_level_sims):.4f}, n={len(same_level_sims)}")
            else:
                logger.info("同层级相似度: 无有效样本")
            if diff_level_sims:
                logger.info(f"跨层级相似度: mean={np.mean(diff_level_sims):.4f}, std={np.std(diff_level_sims):.4f}, n={len(diff_level_sims)}")
            else:
                logger.info("跨层级相似度: 无有效样本")
            ...
            for i in range(n):
                for j in range(n):
                    if i != j:
                        if (units[i] == '册' and units[j] == '节') or (units[i] == '节' and units[j] == '册'):
                            ce_sims.append(sim_matrix[i, j])
                            if len(ce_pairs) < 5:  # 只记录前5对
                                ce_pairs.append((i, j, sim_matrix[i, j]))

            if ce_sims:
                logger.info(f"册-节 相似度: mean={np.mean(ce_sims):.4f}, std={np.std(ce_sims):.4f}, n={len(ce_sims)}")
                logger.info("  示例对:")
                for i, j, sim in ce_pairs:
                    logger.info(f"    [{i}]{texts[i]} <-> [{j}]{texts[j]}: sim={sim:.4f}")

        # 4. 保存 probing 数据
        probing_data = {
            "section_line_ids": [p.get("line_id") for p in section_preds],
            "texts": [p.get("text", "") for p in section_preds],
            "units": units,
            "levels": levels,
            "embeddings": line_features[section_indices].cpu().numpy().tolist(),
            "pred_parents": format_b["pred_parents"],
            "pred_siblings": format_b.get("pred_siblings", []),
        }
        probing_path = os.path.join(task_dir, "probing_data.json")
        with open(probing_path, 'w', encoding='utf-8') as f:
            json.dump(probing_data, f, ensure_ascii=False, indent=2)
        logger.info(f"")
        logger.info(f"[Probing] 数据已保存到: {probing_path}")
        logger.info("=" * 60)
